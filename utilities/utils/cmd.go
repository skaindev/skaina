...
func StartNode(ctx *cli.Context, stack *node.Node) error {
	...
	if err := stack.Service(&skaina); err == nil {
	...
	if mining || ctx.GlobalBool(DeveloperFlag.Name) {
		...
		var skaina *neatptc.NeatIO
		if err := stack.Service(&skaina); err != nil {
			Fatalf("Skaina Blockchain service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := skaina.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}

		skaina.TxPool().SetGasPrice(GlobalBig(ctx, MinerGasPriceFlag.Name))
		if err := skaina.StartMining(true); err != nil {
