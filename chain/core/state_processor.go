	...
	if !neatAbi.IsNeatChainContractAddr(tx.To()) {
		...
		*usedGas += result.UsedGas
		...
		if statedb.GetBalance(from).Cmp(gasValue) < 0 {
			return nil, fmt.Errorf("insufficient SKA for gas (%x). Req %v, has %v", from.Bytes()[:4], gasValue, statedb.GetBalance(from))
		}
		...
		if statedb.GetBalance(from).Cmp(tx.Value()) == -1 {
			return nil, fmt.Errorf("insufficient SKA for tx amount (%x). Req %v, has %v", from.Bytes()[:4], tx.Value(), statedb.GetBalance(from))
		}
