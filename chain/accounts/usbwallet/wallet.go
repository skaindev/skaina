...
type wallet struct {
	...
	deriveChain    skaina.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain skaina.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
