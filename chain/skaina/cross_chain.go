...
import (
	...
	"github.com/skaindev/skaina/chain/consensus/neatcon/epoch"
	...
	"github.com/skaindev/skaina/chain/core/state"
	"github.com/skaindev/skaina/chain/core/types"
	"github.com/skaindev/skaina/chain/log"
	"github.com/skaindev/skaina/chain/trie"
	neatAbi "github.com/skaindev/skaina/neatabi/abi"
	"github.com/skaindev/skaina/neatcli"
	"github.com/skaindev/skaina/neatdb"
	"github.com/skaindev/skaina/neatptc"
	"github.com/skaindev/skaina/network/node"
	"github.com/skaindev/skaina/params"
	"github.com/skaindev/skaina/utilities/common"
	"github.com/skaindev/skaina/utilities/common/math"
...
func (cch *CrossChainHelper) CanCreateSideChain(from common.Address, chainId string, minValidators uint16, minDepositAmount, startupCost *big.Int, startBlock, endBlock *big.Int) error {
	...
	if minDepositAmount.Cmp(officialMinimumDeposit) == -1 {
		return fmt.Errorf("Deposit amount is not meet the minimum official deposit amount (%v SKA)", new(big.Int).Div(officialMinimumDeposit, big.NewInt(params.Nio)))
	}

	if startupCost.Cmp(officialMinimumDeposit) != 0 {
		return fmt.Errorf("Startup cost is not meet the required amount (%v SKA)", new(big.Int).Div(officialMinimumDeposit, big.NewInt(params.Nio)))
	}
	...
	skaina := MustGetNeatChainFromNode(chainMgr.mainChain.NeatNode)
	currentBlock := skaina.BlockChain().CurrentBlock()
...
func (cch *CrossChainHelper) GetEpochFromMainChain() (string, *epoch.Epoch) {
	skaina := MustGetNeatChainFromNode(chainMgr.mainChain.NeatNode)
	var ep *epoch.Epoch
	if neatcon, ok := skaina.Engine().(consensus.NeatCon); ok {
		ep = neatcon.GetEpoch()
	}
	return skaina.ChainConfig().NeatChainId, ep
}
