...
func (cm *ChainManager) InitCrossChainHelper() {
	...
	cm.cch.localTX3CacheDB, _ = rawdb.NewLevelDBDatabase(path.Join(cm.ctx.GlobalString(utils.DataDirFlag.Name), "tx3cache"), 0, 0, "skaina/db/tx3/")
...
func (cm *ChainManager) LoadSideChainInRT(chainId string) {
	...
	var skaina *neatptc.NeatIO
	cm.mainChain.NeatNode.Service(&skaina)
