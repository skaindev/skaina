...
import (
	...
	"github.com/skaindev/skaina/neatabi/abi"
	...
	cmn "github.com/skaindev/common-go"
...
const (
	TotalMintingReward = "32000000000000000000000000" // 32M
	TotalMintingYears  = 10

	DefaultAccountPassword = "skaina"
)
...
func initNeatGenesis(ctx *cli.Context) error {
	...
	log.Infof("this is init-skaina chainId %v", chainId)
	log.Info("this is init-skaina" + ctx.GlobalString(utils.DataDirFlag.Name) + "--" + ctx.Args()[0])
...
func init_neat_genesis(config cfg.Config, balanceStr string, isMainnet bool) error {
	...
	if err != nil {
		utils.Fatalf("init skaina failed")
		return err
	}
...
func init_neatchain(chainId string, neatGenesisPath string, ctx *cli.Context) {
	...
	chainDb, err := rawdb.NewLevelDBDatabase(filepath.Join(utils.MakeDataDir(ctx), chainId, clientIdentifier, "/chaindata"), 0, 0, "skaina/db/chaindata/")
	if err != nil {
...
func init_em_files(config cfg.Config, chainId string, genesisPath string, validators []types.GenesisValidator) error {
	...
	if err != nil {
		utils.Fatalf("failed to read skaina genesis file: %v", err)
		return err
	}
...
func createGenesisDoc(config cfg.Config, chainId string, coreGenesis *core.Genesis, privValidator *types.PrivValidator, validators []types.GenesisValidator) error {
	...
	if _, err := os.Stat(genFile); os.IsNotExist(err) {
		...
		if chainId == MainChain || chainId == TestnetChain {
			rewardPerBlock = big.NewInt(507356671740000000)
		} else {
