...
import (
	...
	"github.com/skaindev/skaina/neatptc/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	pm.logger.Debug("Removin Skaina Blockchain peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	pm.logger.Info("Stopping Skaina protocol")
	...
	pm.logger.Info("Skaina protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Skaina Blockchain peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Skaina Blockchain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Skaina Blockchain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Skaina Blockchain message handling failed", "err", err)
			return err
		}
	}
}
